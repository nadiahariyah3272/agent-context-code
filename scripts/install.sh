...
if [[ -n "${TORCH_INDEX_URL}" ]]; then
  ...
  EXISTING_BUILD=$(uv run --directory "${PROJECT_DIR}" python -c \
  ...
  if [[ "${EXISTING_BUILD}" == "${WANTED_BUILD}" ]]; then
    ...
    msg "Installing GPU-accelerated PyTorch (${GPU_STATUS}) via uv.toml overlay..."
    cat > "${PROJECT_DIR}/uv.toml" <<UVTOML
...
UVTOML
    ...
    if (cd "${PROJECT_DIR}" && uv lock --upgrade-package torch && uv sync); then
      msg "GPU-accelerated PyTorch installed successfully."
    else
      printf "${YELLOW}⚠ GPU PyTorch uv.toml + re-lock failed — trying uv pip install fallback...${NC}\n"
      if (cd "${PROJECT_DIR}" && uv pip install torch --index-url "${TORCH_INDEX_URL}" --reinstall --quiet 2>&1); then
        ...
        printf "${YELLOW}⚠ Note: lockfile may not reflect GPU torch. If torch reverts to CPU, re-run installer.${NC}\n"
      else
