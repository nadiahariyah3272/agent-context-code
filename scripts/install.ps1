...
# pyproject.toml defines a default CPU pytorch index via [tool.uv.sources].
# When a GPU is detected, we write a uv.toml overlay that redirects torch
# resolution to the GPU-specific index (e.g. cu128), then re-lock and sync.
# Because uv.toml overrides the pyproject.toml index, ALL subsequent
# `uv run` and `uv sync` commands will use GPU torch — no --no-sync needed.
# Both uv.toml and uv.lock are gitignored.
#
...
# Install GPU-accelerated PyTorch via uv.toml overlay + re-lock.
# This writes a uv.toml that overrides the pytorch index URL defined in
# pyproject.toml, then re-locks and syncs.  All subsequent `uv run` and
# `uv sync` commands will resolve torch from the GPU index.
if ($torchIndexUrl) {
    $wantedBuild = $torchIndexUrl.Split('/')[-1]  # e.g. "cu128"
    $existingBuild = ""
    try {
        $torchCheck = uv run --directory $ProjectDir python -c "import torch; v=torch.__version__; print(v.split('+')[-1] if '+' in v else 'cpu')" 2>&1
    ...
    if ($existingBuild -eq $wantedBuild) {
        ...
        Write-Host "Installing GPU-accelerated PyTorch ($gpuStatus) via uv.toml overlay..."
        $uvTomlContent = @"
# Auto-generated by installer — overrides pytorch index for GPU wheels.
# To revert to CPU: delete this file or run gpu-setup --cpu
...
explicit = true
"@
        Set-Content -Path (Join-Path $ProjectDir "uv.toml") -Value $uvTomlContent -Encoding UTF8

        Push-Location $ProjectDir
        $lockOk = $false
        $syncOk = $false
        try {
            uv lock --upgrade-package torch 2>&1 | Out-Host
        ...
        if ($lockOk -and $syncOk) {
            Write-Host "GPU-accelerated PyTorch installed successfully."
        }
        else {
            Write-Warning "GPU PyTorch uv.toml + re-lock failed - trying uv pip install fallback..."
            try {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Write-Warning "Note: lockfile may not reflect GPU torch. If torch reverts to CPU, re-run installer."
                }
                else {
                    ...
                    Remove-Item -Force (Join-Path $ProjectDir "uv.toml") -ErrorAction SilentlyContinue
